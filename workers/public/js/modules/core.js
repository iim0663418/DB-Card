// DB-Card Core Module — shared dependencies for all frontend modules
// Extracted from user-portal-init.js (ES module conversion)
...
export const currentLang = userLang.startsWith('zh') ? 'zh' : 'en';
...
// ==================== Global Instances ====================
export const stateManager = new CardStateManager();
export const errorHandler = new ErrorHandler();

// ==================== Helpers ====================
export function getHeadersWithCSRF(baseHeaders = {}) {
const csrfToken = sessionStorage.getItem('csrfToken');
if (csrfToken) {
    return { ...baseHeaders, 'X-CSRF-Token': csrfToken };
}
return baseHeaders;
}
...
// previewLang: shared mutable state for preview language
...
// Forward declaration — will be set by consent module
let _showConsentModal = null;
let _showRestoreConsentModal = null;

export function registerConsentHandlers(showConsent, showRestore) {
_showConsentModal = showConsent;
_showRestoreConsentModal = showRestore;
}

export async function validateSessionAndConsent() {
try {
    const response = await apiCall('/api/consent/check', { method: 'GET' });
    const data = response.data || response;

    if (data.needs_consent) {
        if (_showConsentModal) _showConsentModal(data.current_policy, data.reason);
        return { valid: true, consentOk: false };
    }

    if (data.is_withdrawn && data.can_restore) {
        if (_showRestoreConsentModal) _showRestoreConsentModal(data.days_remaining);
        return { valid: true, consentOk: false };
    }
...
// Forward declaration — will be set by self-card-ocr module
let _renderSelectionPage = null;

export function registerRenderSelectionPage(fn) {
_renderSelectionPage = fn;
}
...
try {
    ...
    if (_renderSelectionPage) _renderSelectionPage();
} catch (err) {
    handleError(err);
}
}
