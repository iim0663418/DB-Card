...
export async function csrfMiddleware(request: Request, env: Env): Promise<Response | null> {
  ...
  if (DEBUG) console.log('[CSRF] HTTP method:', method);
  ...
  if (DEBUG) console.log('[CSRF] X-CSRF-Token header exists:', !!csrfToken);
  if (DEBUG) console.log('[CSRF] X-CSRF-Token value:', csrfToken ? csrfToken.substring(0, 8) + '...' : 'null');
  if (!csrfToken) {
  ...
  if (DEBUG) console.log('[CSRF] Cookie header exists:', !!cookieHeader);
  if (!cookieHeader) {
  ...
  if (DEBUG) console.log('[CSRF] Request path:', url.pathname);
  if (DEBUG) console.log('[CSRF] Is admin path:', isAdminPath);
  if (DEBUG) console.log('[CSRF] Selected token type:', isAdminPath ? 'admin_token' : 'auth_token');
  if (DEBUG) console.log('[CSRF] Session token extracted:', sessionToken ? sessionToken.substring(0, 8) + '...' : 'null');
  if (!sessionToken) {
  ...
  if (DEBUG) console.log('[CSRF] Stored token from KV:', storedToken ? storedToken.substring(0, 8) + '...' : 'null');
  if (!storedToken) {
  ...
  try {
    ...
    if (DEBUG) console.log('[CSRF] Token comparison result:', isValid);
    if (!isValid) {
      return errorResponse('csrf_token_invalid', 'Invalid CSRF token', 403, request);
    }
  } catch (error) {
